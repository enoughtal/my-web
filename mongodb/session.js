    ...
    */
    return class MongodbStore extends Store {
        ...
        async set(sid, session, cb) {
            ...
            try {
                await operateInCollection(operateUpdateOne(filter, doc, opts), this.collName)
            }
            catch (err) {
                return cb(err)
            }
        ...
        async get(sid, cb) {
            let result
            try {
                result = await operateInCollection(operateFindAndToArray({ sid }), this.collName)
            }
            catch (err) {
                return cb(err)
            }
        ...
        async all(cb) {
            let result
            try {
                result = await operateInCollection(operateFindAndToArray(), this.collName)
            }
            catch (err) {
                return cb(err)
            }
            cb(undefined, result.map(doc => doc.session))
        }
