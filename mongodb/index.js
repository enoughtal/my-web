...
async function initializeExpiredIndex(collName) {
    const existed = await operateInCollection(operateIndexExists('expiredAt_1'), collName)
    debug('db:init:indexExists')(existed)
    if (existed) return

    const fields = { expiredAt: 1 }
    const opts = { expireAfterSeconds: 0 }
    const result = await operateInCollection(operateCreateIndex(fields, opts), collName)
