...
async function readTodos(req, res, next) {
  ...
  try {
    const todos = await operateInCollection(
      operateFindAndToArray(filter, sort),
      COLL_NAME
    );
...
async function upsertTodos(req, res, next) {
  ...
  try {
    const upsertResult = await operateInCollection(
      operateUpsertMany(ids, entities),
      COLL_NAME
    );
    const deleteResult = await operateInCollection(
      operateDeleteMany(deleteFilter),
      COLL_NAME
    );
