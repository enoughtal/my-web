...
async function readUsers(req, res, next) {
  try {
    const users = await operateInCollection(
      operateFindAndToArray({}),
      COLL_NAME
    );
...
async function register(req, res, next) {
  ...
  if (
    req._existedUsers.map((user) => user.username).includes(newUser.username)
  ) {
    res.send({
      code: '11',
      msg: '用户名已存在',
...
async function login(req, res, next) {
  ...
  const index = req._existedUsers
    .map((user) => user.username)
    .indexOf(comingUser.username);
...
export async function _whoVisit(req) {
  ...
  if (userId !== GUEST_ID) {
    ...
    try {
      const user = await operateInCollection(
        operateFindOne(filter, opts),
        COLL_NAME
      );
      debug('db:users')(user);
