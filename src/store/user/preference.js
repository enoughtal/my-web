...
const gain = (result) => {
  ...
  switch (result) {
    case 'draw-easy':
      return { ...gained, score: scoreValues[0] };
    case 'draw-hard':
      return { ...gained, score: scoreValues[1] };
    case 'win-easy':
      return { ...gained, score: scoreValues[2] };
    case 'win-hard':
      return { ...gained, score: scoreValues[3] };
    default:
      return { ...gained, lose: 1 };
  }
};
...
/* 保存游戏得分 */
const saveTic = createAsyncThunk('user/server/saveTic', async (data) => {
...
const selectPower = createSelector(
  ...
  ({ totalScore, lose, count }) =>
    count ? Math.floor(totalScore * (1 - lose / count)) : 0
);

/* selector，计算头衔 */
const selectTitle = createSelector(selectPower, (power) => {
...
const selectNotLoseRate = createSelector(
  (state) => state.user.tic,
  ({ lose, count }) =>
    count ? ((1 - lose / count) * 100).toFixed(1) + '%' : '-'
);

/* reducers */
const preferenceReducers = {
  changeElement: (state) => {
    state.preference.element =
      ELEMENT[(ELEMENT.indexOf(state.preference.element) + 1) % ELEMENT.length];
  },
  toggleTheme: (state) => {
