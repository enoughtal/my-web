...
export default function TodosTable() {
    ...
    */
    ...
    /* 保存todos */
    const save = (tx) => {//tx由useBlocker传参，手动保存时没有
        if (dirtyTodos) {
    ...
    /* 弹出提示，提醒保存 */
    const prompt = (tx) => {
        const message = '数据已修改，是否保存？\r\n点击“确定”进入登录页面，点击“取消”不保存'

        if (tx && tx.location.pathname === LOGIN_PATH) {//如果直接点击登录就不提示
            tx.redirect(LOGIN_PATH, { state: { from: location } })
        }
        else {
            if (window.confirm(message)) {//确定
                if (tx) {
    ...
    const manualSave = () => {
        if (!locked) {
            lock()
            if (!isAuth) {//手动保存不判断isDirty
                prompt()
            }
            else {
                save()

                const data = {//额外保存element
                    element: el,
                    userId
                }
    ...
    const saveButton =
        <div className='myapp-todos-table-header-save none-user-select'
            onClick={manualSave}
        >
            💾
        </div>
    ...
    const columns = [
        ...
        },
        {
            ...
            onFilter(value, record) { return value === record.rank },
            sorter(a, b) { return b.rank - a.rank },
        },
        {
            title: '创建时间',
    ...
    const components = {
        header: {
            cell: (props) => (
                ...
                <th id={themeId(theme, 'myapp-todos-table-header-cell')}
                    {...props}
                />
            )
        },
        body: {
            cell: (props) => (
                <td id='myapp-todos-table-body-cell'
                    {...props}
                />
            )
        }
