...
1. 掌握一般编程语言的主要概念。理解原始类型和引用类型（对象）；理解传值和传引用；理解变量的可变与不可变；理解相等性判断；理解类型、静态语言动态语言的区别，能看懂typescript；理解栈内存和堆内存（前者存放对象方法，后者存放实例属性）；理解表达式和函数式编程；理解纯函数和副作用；理解递归函数；理解垃圾回收等。理解算法复杂度。会用正则表达式。
