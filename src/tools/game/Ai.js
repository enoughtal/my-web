...
class Ai {
  ...
  output() {
    const result = this.lines
      ...
      //.log()
      .reduce((prev, cur) => {
        new MyObj(cur).entries().forEach(([type, points]) => {
          new MyObj(points).entries().forEach(([point, value]) => {
            if (prev[type]) {
      ...
      //.log()
      .reduce((prev, cur) => {
        new MyObj(cur).entries().forEach(([value, entry]) => {
          new MyObj(entry).entries().forEach(([type, point]) => {
            if (prev[value]) {
    ...
    if (!best) {
      //如果没有可以获胜的解了，边界情况，已经是平局，但还有格子可走
    ...
    if (this.level === 'easy') {
      //简单的ai会把better混进去
      if (this._random(10) === 0) {
        //10中1
        const bad = this._randomChoseInArray(
          Object.values(better || {}).flat()
        );
        if (bad) {
