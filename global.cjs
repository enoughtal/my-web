...
function getTls() {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    tls = {
      ...
      key: fs.readFileSync(
        path.join(process.cwd(), './private/localhost/localhost-key.pem')
      ),
      cert: fs.readFileSync(
        path.join(process.cwd(), './private/localhost/localhost.pem')
      ),
    };
  } else {
    tls = {
      pfx: fs.readFileSync(
        path.join(process.cwd(), './private/hueyond.run_iis/hueyond.run.pfx')
      ),
      passphrase: fs.readFileSync(
        path.join(process.cwd(), './private/hueyond.run_iis/keystorePass.txt'),
        'utf8'
      ),
    };
  }
